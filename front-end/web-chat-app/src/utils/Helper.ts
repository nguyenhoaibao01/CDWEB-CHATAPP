...
const Helper = {
  ...
  getAuthrefreshToken: (): string | null => localStorage.getItem('op_refreshToken'),
  storeAuthrefreshToken: (token): void => localStorage.setItem('op_refreshToken', token),
  removeAuthrefreshToken: (): void => localStorage.removeItem('op_refreshToken'),
  ...
  convertMessage: (res: any) => {
    if ((res.code && res.message) || res.message === 'failed') {
      ...
      const textFirst = `${messageContent.charAt(0).toUpperCase()}${messageContent.slice(1)}`;
